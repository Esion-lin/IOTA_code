...
public class NativeSecp256k1Test {
    private static Hotstuff hotstuff = new Hotstuff();
    ...
    public static void main(String[] args) throws AssertFailException{
        ...
        for(int i = 0; i < 4; i++){
            hotstuff.call_send("127.0.0.1", 10060 + i, "2A2A67007A926E6594AF3EB564FC7400", hotstuff.getIdx());
        }
        ...
        int itr = 32;
        for(int i = 0; i < 3; i++){
            int seq = (int)result[itr];
            System.out.println("number of node:" + String.valueOf(seq));
            itr ++;
            int len = (int)result[itr];
            System.out.println("leng of sig:" + String.valueOf(len));
            itr ++;
            byte[] sig = new byte[len];
            System.arraycopy(result, itr, sig, 0, len);
            for(int j = 0; j < len; j++){
                System.out.print(" " + String.valueOf(sig[j]));
            }
            itr += len;
            byte[] pub = BaseEncoding.base16().lowerCase().decode(pubstr[seq]);
            System.out.println(String.valueOf(NativeSecp256k1.verify( data, sig, pub)));
        }
